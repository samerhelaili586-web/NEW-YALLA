...
def get_max_elapsed_minutes(target_date, now_dt=None, client_tz_offset=None):
    ...
    if target_date < today:
        # No 8-hour daily limit for past days, allow up to 24 hours.
        weekday = target_date.weekday()
        if weekday == 6:  # Sunday
            return 0  # Sunday is day off
        return 24 * 60
    ...
    if weekday == 5:  # Saturday (08:30 onwards)
        start = 8 * 60 + 30  # 08:30
        if cur_mins <= start:
            return 0
        return cur_mins - start

    # Mon-Fri (08:30 onwards, excluding 13:00-14:00 lunch break)
    start = 8 * 60 + 30    # 08:30
    lunch_s = 13 * 60      # 13:00
    lunch_e = 14 * 60      # 14:00
